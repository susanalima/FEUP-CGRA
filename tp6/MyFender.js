...
class MyFender extends MyCylinder
{
	constructor(scene, slices, stacks, rad,length)
	{
		super(scene,slices,stacks,rad,length);
	...
	initBuffers()
	{
		...
		while(count < this.slices)
		{
			...
			for(var i = 0; i <= this.stacks/2.0; i++)
			{
		...
		for (var i = 0; i < this.slices*(this.stacks+1)/2; i+=(this.stacks+1))
		{
